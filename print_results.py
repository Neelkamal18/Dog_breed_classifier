...
def print_results(results_dic, results_stats_dic, model, 
    ...
    for key in results_stats_dic:
        # Print out all percentage statistics in the dictionary
        if key.startswith('pct'):
            print("{:20}: {:.2f}%".format(key, results_stats_dic[key]))

    # IF print_incorrect_dogs == True AND there were incorrect dog classifications
    if (print_incorrect_dogs and 
        (results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] != results_stats_dic['n_images'])):
        print("\nINCORRECT Dog/NOT Dog Assignments:")
        for key in results_dic:
            # Print pet label and classifier label for misclassified dogs
            if (results_dic[key][3] == 1 and results_dic[key][4] == 0) or \
               (results_dic[key][3] == 0 and results_dic[key][4] == 1):
                print("Real: {:>26}   Classifier: {:>30}".format(results_dic[key][0],
                                                                  results_dic[key][1]))

    # IF print_incorrect_breed == True AND there were incorrect breed classifications
